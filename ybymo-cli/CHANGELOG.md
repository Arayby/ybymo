...
- `VersionProvider` para exibição dinâmica da versão do CLI via PicoCLI (`--version`) com fallback para `unknown`.
- Testes dedicados para `VersionProvider` cobrindo formato e conteúdo da saída de versão.
- Builder de arquivo CSV para testes do CLI (`CsvTestFileBuilder`) para simplificar criação de cenários.

### Alterado

- Versionamento independente do módulo `ybymo-cli` com seu próprio CHANGELOG.
- `build.gradle` do módulo atualizado com plugin `application`, alias do plugin GraalVM, version catalog (`libs.*`) e `mainClass` centralizado.
- Manifesto dos artefatos (`jar` e `fatJar`) passou a incluir `Implementation-Version` além de `Main-Class`.
- Comando raiz (`Main`) passou a usar `versionProvider = VersionProvider.class` em vez de versão fixa hardcoded.
- Testes de `CsvCommand` foram refatorados para usar builders de fixture e ganhar legibilidade/manutenção.
- Testes do módulo CLI foram ajustados para padrão de nomenclatura `method_scenario_expectedResult`.
...
[1.1.3]: https://github.com/arayby/ybymo/releases/tag/v1.1.3
